...
namespace Models\UserSettings\Map;
...
class UserSettingsTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Models.UserSettings.Map.UserSettingsTableMap';
     ...
     */
    const OM_CLASS = '\\Models\\UserSettings\\UserSettings';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Models.UserSettings.UserSettings';
    ...
    /**
     * the column name for the auto_play field
     */
    const COL_AUTO_PLAY = 'user_settings.auto_play';

    /**
     * the column name for the plan_id field
     */
    const COL_PLAN_ID = 'user_settings.plan_id';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('Id', 'UserId', 'AutoPlay', 'PlanId', ),
        self::TYPE_CAMELNAME     => array('id', 'userId', 'autoPlay', 'planId', ),
        self::TYPE_COLNAME       => array(UserSettingsTableMap::COL_ID, UserSettingsTableMap::COL_USER_ID, UserSettingsTableMap::COL_AUTO_PLAY, UserSettingsTableMap::COL_PLAN_ID, ),
        self::TYPE_FIELDNAME     => array('id', 'user_id', 'auto_play', 'plan_id', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('Id' => 0, 'UserId' => 1, 'AutoPlay' => 2, 'PlanId' => 3, ),
        self::TYPE_CAMELNAME     => array('id' => 0, 'userId' => 1, 'autoPlay' => 2, 'planId' => 3, ),
        self::TYPE_COLNAME       => array(UserSettingsTableMap::COL_ID => 0, UserSettingsTableMap::COL_USER_ID => 1, UserSettingsTableMap::COL_AUTO_PLAY => 2, UserSettingsTableMap::COL_PLAN_ID => 3, ),
        self::TYPE_FIELDNAME     => array('id' => 0, 'user_id' => 1, 'auto_play' => 2, 'plan_id' => 3, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Models\\UserSettings\\UserSettings');
        $this->setPackage('Models.UserSettings');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id', 'Id', 'VARCHAR', true, null, null);
        $this->addForeignKey('user_id', 'UserId', 'VARCHAR', 'users', 'id', true, null, null);
        $this->addColumn('auto_play', 'AutoPlay', 'TINYINT', true, null, null);
        $this->addForeignKey('plan_id', 'PlanId', 'VARCHAR', 'plans', 'id', true, null, null);
    } // initialize()
  ...
  array (
    ...
    1 => ':id',
        ...
        $this->addRelation('Plans', '\\Models\\Plans\\Plans', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    0 => ':plan_id',
    1 => ':id',
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(UserSettingsTableMap::COL_ID);
            $criteria->addSelectColumn(UserSettingsTableMap::COL_USER_ID);
            $criteria->addSelectColumn(UserSettingsTableMap::COL_AUTO_PLAY);
            $criteria->addSelectColumn(UserSettingsTableMap::COL_PLAN_ID);
        } else {
            $criteria->addSelectColumn($alias . '.id');
            $criteria->addSelectColumn($alias . '.user_id');
            $criteria->addSelectColumn($alias . '.auto_play');
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Models\UserSettings\UserSettings) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(UserSettingsTableMap::DATABASE_NAME);
            $criteria->add(UserSettingsTableMap::COL_ID, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(UserSettingsTableMap::COL_ID) && $criteria->keyContainsValue(UserSettingsTableMap::COL_ID) ) {
